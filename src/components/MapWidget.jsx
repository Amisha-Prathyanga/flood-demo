...
const DSD_URL =
  "https://services1.arcgis.com/tMAq108b7itjkui5/arcgis/rest/services/SL_DSD_codes/FeatureServer/0";
const SL_FLOOD_2025_URL =
  "https://services1.arcgis.com/tMAq108b7itjkui5/arcgis/rest/services/Kalani_Attanagalu_Daduru_Maoya_flood_extent/FeatureServer/46";
const PAST_FLOOD_URL =
  "https://services1.arcgis.com/tMAq108b7itjkui5/arcgis/rest/services/Past_flood_events/FeatureServer/34";
...
const MapWidget = forwardRef(({ filters, onPointAnalysis }, ref) => {
  ...
  const onLegendMouseDown = useCallback((e) => {
    ...
    const onMove = (ev) => {
      ...
      setLegendPos({
        left: dragging.current.ox + dx,
        top: dragging.current.oy + dy,
        right: "auto",
        bottom: "auto",
      });
    };
  ...
  const dropPinAndQuery = useCallback(
    async (lng, lat) => {
  ...
  useEffect(() => {
    ...
    const dsdLayer = new FeatureLayer({
      ...
      renderer: {
        ...
        symbol: {
          type: "simple-fill",
          color: [173, 216, 230, 0],
          outline: { color: [60, 100, 160, 0.85], width: 1 },
        },
      },
    });
    const flood2025Layer = new FeatureLayer({
      ...
      renderer: {
        ...
        symbol: {
          type: "simple-fill",
          color: [30, 80, 200, 0.35],
          outline: { color: [59, 130, 246, 0.9], width: 1.2 },
        },
      },
    });
    ...
    view.when(() => {
      view.on("click", (e) =>
        dropPinAndQuery(e.mapPoint.longitude, e.mapPoint.latitude),
      );

      // ScaleBar
      view.ui.add(
        new ScaleBar({ view, unit: "metric", style: "line" }),
        "bottom-left",
      );
